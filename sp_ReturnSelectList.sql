DROP PROCEDURE sp_ReturnSelectList
GO
CREATE PROCEDURE sp_ReturnSelectList
	 @JsonTable NVARCHAR(100)
	, @JsonColumn NVARCHAR(100)
	, @JsonType INT
	, @SelectList NVARCHAR(4000) OUTPUT
AS
BEGIN
	...
	@SelectList = 
		STRING_AGG(
			CASE 
				...
				WHEN [type] = 4 THEN CONCAT('[',[key],']')
				WHEN [type] = 5 THEN CONCAT('[',[key],']')
				ELSE NULL
			END
			, CHAR(13)+CHAR(10)+','
		)
